...
def test_get_threads_and_find_by_name(db):
    ...
    with db._get_connection() as conn:
        ...
        cursor.execute("INSERT INTO threads (name, icon, is_active, is_archived) VALUES (?, ?, ?, ?)", ("Beta", "b", 0, 1))
        conn.commit()
...
def test_update_thread(db):
    with db._get_connection() as conn:
        ...
        thread_id = cursor.lastrowid
        conn.commit()
    updated = db.update_thread(thread_id, name="Gamma2", is_archived=1, tags=["tag1","tag2"], metadata={"foo":1})
    assert updated
    thread = db.get_thread(thread_id)
...
def test_get_thread_message_counts(db):
    with db._get_connection() as conn:
        ...
        thread_id = cursor.lastrowid
        cursor.execute("INSERT INTO messages (thread_id, sender, content) VALUES (?, ?, ?)", (thread_id, "user", "msg1"))
        cursor.execute("INSERT INTO messages (thread_id, sender, content) VALUES (?, ?, ?)", (thread_id, "ai", "msg2"))
        conn.commit()
...
def test_add_message_with_attachments(db):
    with db._get_connection() as conn:
        ...
        thread_id = cursor.lastrowid
        conn.commit()
    attachments = [{"file_name": "f.txt", "file_path": "/tmp/f.txt", "file_size": 1, "mime_type": "text/plain", "hash": "abc"}]
...
def test_update_message(db):
    with db._get_connection() as conn:
        ...
        thread_id = cursor.lastrowid
        cursor.execute("INSERT INTO messages (thread_id, sender, content) VALUES (?, ?, ?)", (thread_id, "user", "old"))
...
def test_update_message_no_fields(db):
    with db._get_connection() as conn:
        ...
        thread_id = cursor.lastrowid
        cursor.execute("INSERT INTO messages (thread_id, sender, content) VALUES (?, ?, ?)", (thread_id, "user", "msg"))
        msg_id = cursor.lastrowid
        conn.commit()
...
def test_cleanup_old_messages(db):
    with db._get_connection() as conn:
        ...
        thread_id = cursor.lastrowid
        cursor.execute("INSERT INTO messages (thread_id, sender, content, timestamp) VALUES (?, ?, ?, datetime('now', '-40 days'))", (thread_id, "user", "oldmsg"))
        conn.commit()

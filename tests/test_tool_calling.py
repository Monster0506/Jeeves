...
class TestThreadIdentifierResolution:
    ...
    def test_thread_identifier_edge_cases(self, setup_tools):
        ...
        long_name = "A" * 1000
        thread_id = chat_manager.create_thread(long_name, "🧪")
        result = tools._resolve_thread_identifier(long_name)
        assert result == thread_id
    ...
    def test_thread_identifier_with_special_characters(self, setup_tools):
        ...
        # Test resolution with special characters
        result = tools._resolve_thread_identifier("Thread with @#$%^&*()")
        assert result == thread_id

        # Test partial match with special characters
        result = tools._resolve_thread_identifier("Thread with")
        assert result == thread_id
...
class TestGeminiProviderToolCalling:
    ...
    def test_gemini_provider_tool_registration(self, mock_client_class):
        """Test tool registration with Gemini provider."""
        # Mock the client
        mock_client = Mock()
        ...
        assert "calculate_sum" in provider.registered_tools
        assert "get_weather" in provider.registered_tools
        assert provider.registered_tools["calculate_sum"] == calculate_sum
        assert provider.registered_tools["get_weather"] == get_weather
    ...
    def test_gemini_provider_build_tools_config(self, mock_client_class):
        ...
        # With automatic function calling, tools are Python functions directly
    ...
    def test_gemini_provider_generate_response_with_tools(self, mock_client_class):
        ...
        provider = GeminiProvider({"api_key": "test_key", "enable_tool_calling": True})
        provider.initialize()
    ...
    def test_gemini_provider_generate_response_with_function_calls(self, mock_client_class):
        ...
        # Mock successful response with automatic function calling
        mock_response = Mock()
        ...
        # Verify that generate_content was called with tools
        mock_client.models.generate_content.assert_called_once()
        call_args = mock_client.models.generate_content.call_args
        config = call_args[1]["config"]
        assert hasattr(config, "tools")
    ...
    def test_gemini_provider_generate_response_tool_calling_disabled(self, mock_client_class):
        ...
        provider = GeminiProvider({"api_key": "test_key", "enable_tool_calling": False})
        provider.initialize()
    ...
    def test_gemini_provider_handle_function_calls_with_error(self, mock_client_class):
        """Test automatic function calling when tool execution fails."""
        # Mock the client
        mock_client = Mock()
        mock_client_class.return_value = mock_client
        mock_models = [Mock(), Mock()]
        ...
        # Mock response indicating an error occurred
        mock_response = Mock()
    ...
    def test_gemini_provider_validate_config_with_tool_calling(self):
        ...
        provider = GeminiProvider({"api_key": "test_key", "max_tool_calls": 5})
...
class TestToolCallingIntegration:
    ...
    def test_full_tool_calling_workflow(self, mock_client_class):
        """Test complete tool calling workflow with automatic function calling."""
        # Mock the client
        mock_client = Mock()
        mock_client_class.return_value = mock_client
        mock_models = [Mock(), Mock()]
    ...
    def test_multiple_tool_calls_in_single_request(self, mock_client_class):
        """Test multiple tool calls in a single request with automatic function calling."""
        # Mock the client
        mock_client = Mock()
        mock_client_class.return_value = mock_client
        mock_models = [Mock(), Mock()]
    ...
    def test_tool_calling_with_context(self, mock_client_class):
        """Test tool calling with conversation context using automatic function calling."""
        # Mock the client
        mock_client = Mock()
        mock_client_class.return_value = mock_client
        mock_models = [Mock(), Mock()]
...
class TestToolCallingImplementationExamples:
    """Implementation-style tests for tool calling based on TOOL_CALLING_GUIDE.md examples."""

    def test_log_thought_tool(self):
        manager = AIProviderManager()
        thoughts = []
    ...
    def test_search_files_tool(self, tmp_path):
        # Setup test files
        (tmp_path / "foo.txt").write_text("test")
        (tmp_path / "bar.py").write_text("print('hi')")
        (tmp_path / "baz.py").write_text("print('bye')")
        manager = AIProviderManager()
        ...
        def search_files(query: str, directory: str = ".") -> str:
            ...
            if results:
                return f"Found {len(results)} files: {', '.join(sorted(results))}"
            else:
    ...
    def test_tool_chaining(self):
        ...
        manager.register_tool("get_user_info", get_user_info)
    ...
    def test_contextual_action_tool(self):
        ...
        def create_file(context: str) -> str:
            return f"File created for {context}"

        def search_content(context: str) -> str:
            return f"Searched content for {context}"
    ...
    def test_manual_tool_execution_and_error_handling(self):
        manager = AIProviderManager()
        ...
        manager.register_tool("safe_divide", safe_divide)
        # Valid
        result = manager.execute_tool("safe_divide", {"a": 10, "b": 2})
        assert result == 5.0
        # Error
        with pytest.raises(ValueError, match="Division by zero"):
            manager.execute_tool("safe_divide", {"a": 10, "b": 0})
